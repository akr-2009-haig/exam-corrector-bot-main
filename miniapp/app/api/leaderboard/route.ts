 ...
 * valid signature the API refuses (unless ALLOW_ANON=1, for local testing).
 *
...
/** Start of the current week (Saturday) in Baghdad time, as a UTC ms stamp. */
function startOfWeek(now: number): number {
  const local = new Date(now + BAGHDAD_OFFSET_MS);
  const sinceSaturday = (local.getUTCDay() + 1) % 7; // Sat→0 … Fri→6
  return (
    Date.UTC(
      local.getUTCFullYear(),
      local.getUTCMonth(),
      local.getUTCDate() - sinceSaturday,
    ) - BAGHDAD_OFFSET_MS
  );
}
...
function startOfMonth(now: number): number {
  const local = new Date(now + BAGHDAD_OFFSET_MS);
  return Date.UTC(local.getUTCFullYear(), local.getUTCMonth(), 1) - BAGHDAD_OFFSET_MS;
}
...
function rank(entries: LeaderboardEntry[], meId: number | null): PeriodData {
  ...
  const me =
    meIdx >= 0
      ? {
          ...ranked[meIdx]!,
          // Points needed to overtake the student directly above (0 at #1).
          gap:
            meIdx > 0
              ? Math.max(
                  1,
                  Math.ceil(ranked[meIdx - 1]!.points - ranked[meIdx]!.points),
                )
              : 0,
        }
      : null;
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (!user && process.env.ALLOW_ANON !== "1") {
    return NextResponse.json(
      { error: "افتح هذه الصفحة من داخل تيليجرام" },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    // Compare the all-time standing with what the user saw on their LAST
    // visit; an improvement triggers the celebration overlay once.
    let celebration: {
      prevRank: number;
      newRank: number;
      pointsGained: number;
    ...
    if (meId !== null && all.me) {
      ...
      const gained = prev
        ? Math.round((all.me.points - prev.points) * 10) / 10
        : 0;
